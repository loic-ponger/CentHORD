...
def select_non_overlapping(diagonals_df):
...
def plot_heatmap(dist_matrix, nonoverlapping_df, checker_zones, ncc_scores,
                 output_png, invert_colors=False, min_diag_len=4):
    """Plot heatmap + diagonals + checkerboard zones + NCC barplot."""
    cmap_name = "YlOrRd" if not invert_colors else "RdYlBu_r"
        ...
        for _, row in nonoverlapping_df.iterrows():
            if row["length"] >= min_diag_len:
                ax.plot([row["start_col"], row["end_col"]],
                        ...
                        color="black", lw=2)
                ax.text(row["mid_col"], row["mid_row"], str(int(row["length"])),
...
def main():
    ...
    if args.detect_diagonals:
        ...
        nonoverlapping_df = select_non_overlapping(diagonals_df)
        ...
        plot_heatmap(dist_matrix, nonoverlapping_df, checker_zones, ncc_scores,
