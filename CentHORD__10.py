...
def compute_region_mean_distance(dist_matrix, row, exclude_diag=True):
    """
    Compute the mean distance in the square region defined by a diagonal.
    Optionally exclude the diagonal itself from the mean.
    """
    ...
    if exclude_diag:
        ...
        np.fill_diagonal(mask, False)  # masque la diagonale
        values = region[mask]
    else:
        values = region.ravel()
...
def compute_diagonal_stats(dist_matrix, row, neighbor_dist=3):
    """
    Calcule plusieurs statistiques pour une diagonale :
    - moyenne, médiane et variance des distances sur la diagonale
    - moyenne et médiane des distances dans les régions adjacentes
      (gauche, droite, haut, bas) sur une distance de monomers donnée
    """
    r1, r2 = int(row["start_row"]), int(row["end_row"])
    c1, c2 = int(row["start_col"]), int(row["end_col"])
    nrows, ncols = dist_matrix.shape

    # Sous-matrice de la diagonale
    diag_region = dist_matrix[r1:r2+1, c1:c2+1]
    diag_values = diag_region[np.triu_indices_from(diag_region)]
    ...
    diag_median = float(np.median(diag_values))
    diag_var = float(np.var(diag_values))

    # Zones adjacentes
    neighbors = []
    for dr in [-neighbor_dist, 0, neighbor_dist]:
        for dc in [-neighbor_dist, 0, neighbor_dist]:
            if dr == 0 and dc == 0:
                continue
            rr1 = max(0, r1 + dr)
            rr2 = min(nrows - 1, r2 + dr)
            cc1 = max(0, c1 + dc)
            cc2 = min(ncols - 1, c2 + dc)
            region = dist_matrix[rr1:rr2+1, cc1:cc2+1]
            vals = region[~np.isnan(region)]
            if vals.size > 0:
                neighbors.extend(vals)

    if len(neighbors) == 0:
        return [diag_mean, diag_median, diag_var, np.nan, np.nan]

    neighbor_mean = float(np.mean(neighbors))
    neighbor_median = float(np.median(neighbors))

    return [diag_mean, diag_median, diag_var, neighbor_mean, neighbor_median]
...
def plot_heatmap(dist_matrix, nonoverlapping_df, diagonals_df, checker_zones, ncc_scores,
                 output_png, invert_colors=False, min_diag_len=4, diag_offset=1, diag_width=3, diag_color="red", dist_min=-1, dist_max=-1, dpi=300):
    ...
    if dist_min == -1:
        ...
        im = ax.imshow(dist_matrix, origin="upper", cmap=cmap, aspect="equal", vmin=dist_min, vmax=dist_max)

    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="5%", pad=0.05)
    cbar = fig.colorbar(im, cax=cax)
    cbar.set_label("Distance", rotation=270, labelpad=15)
    cbar.ax.yaxis.set_major_locator(ticker.MaxNLocator(nbins=6))  # ~6 ticks
    cbar.ax.tick_params(labelsize=8, length=3)  # taille du texte et des ticks    
    # Diagonals
...
def main():
    ...
    parser.add_argument("--vmin", type=float, default=-1,
    ...
    parser.add_argument("--local_size", type=int, default=5, help="Local window size for diagonal stats")
    ...
    parser.add_argument("--neighbor_distance", type=int, default=3,
                    help="Distance of monomers around the diagonals to calculate the locale statistics")
    ...
    parser.add_argument("--dpi", type=int, default=300, help="DPI for heatmap")
 ...
 # Apply to all diagonals
        if diagonals_df.empty:
            empty_cols = ["diag_id", "start_row", "end_row", "start_col", "end_col", "length",
                  "mid_row", "mid_col", "mean_distance", "region_mean_distance",
                  "region_mean_distance_full", "diag_mean", "diag_median", "diag_var",
                  "neighbor_mean", "neighbor_median"]
            pd.DataFrame(columns=empty_cols).to_csv(prefix + "_diagonals.csv", index=False, sep=" ")
        else:
            diagonals_df["region_mean_distance"] = diagonals_df.apply(
                lambda row: compute_region_mean_distance(dist_matrix, row), axis=1
            )
            diagonals_df["region_mean_distance_full"] = diagonals_df.apply(
                lambda row: compute_region_mean_distance(dist_matrix, row, False), axis=1
            )

            diag_stats = diagonals_df.apply(
                lambda row: compute_diagonal_stats(dist_matrix, row, neighbor_dist=args.neighbor_distance),
                axis=1, result_type="expand"
            )
            diag_stats.columns = ["diag_mean", "diag_median", "diag_var", "neighbor_mean", "neighbor_median"]
            diagonals_df = pd.concat([diagonals_df, diag_stats], axis=1)
            diagonals_df.to_csv(prefix + "_diagonals.csv", index=False, sep=" ")



        # if diagonals_df.empty:
    # # Crée un CSV vide avec uniquement les entêtes
                # empty_cols = ["diag_id", "start_row", "end_row", "start_col", "end_col", "length",
                          # "mid_row", "mid_col", "mean_distance", "region_mean_distance", "region_mean_distance_full"]
                # pd.DataFrame(columns=empty_cols).to_csv(prefix + "_diagonals.csv", index=False, sep=" ")
        # else:
                # diagonals_df["region_mean_distance"] = diagonals_df.apply(
                    # lambda row: compute_region_mean_distance(dist_matrix, row), axis=1
        ...
        nonoverlapping_df.to_csv(prefix + "_nonoverlapping_diagonals.csv", index=False, sep=" ")
    ...
    if args.detect_checkerboard:
            ...
            pd.DataFrame(results, columns=["r1", "c1", "r2", "c2", "mean_ncc"]).to_csv(
                prefix + "_checkerboard_zones.csv", index=False, sep=" ")
    ...
    plot_heatmap(dist_matrix, nonoverlapping_df, diagonals_df, checker_zones, ncc_scores,
                  ...
                  min_diag_len=args.min_diag_len, diag_offset=args.diag_offset, diag_color=args.diag_color, diag_width=args.diag_width, dist_min=args.vmin, dist_max=args.vmax, dpi=args.dpi)
